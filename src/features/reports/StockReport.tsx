...
// Pur habillage de présentation autour de useStockMovements (déjà utilisé par StockPage.tsx,
// filtres période/lot/magasin/produit/direction/péremption/provenance/destination déjà
// complets) -- aucune nouvelle requête. Les formulaires "Enregistrer un mouvement"/
// "Transférer" restent exclusivement sur /stock, opérationnel ; cet état est lecture seule.
export function StockReport() {
  ...
  async function handleExportExcel() {
    ...
    await exportRowsToExcel(
      `etat-stock-${new Date().toISOString().slice(0, 10)}.xlsx`,
      [
  ...
  return (
    <Card>
      <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
        <h2 className="text-base font-semibold text-gray-800">Mouvements de stock</h2>
        <ReportExportButton onExport={handleExportExcel} disabled={!movements || movements.length === 0} />
      </div>

      <div className="mb-4 flex flex-wrap items-end gap-3">
        <ReportDateRangeFilter
          idPrefix="stock-report"
          startDate={dateFrom}
          endDate={dateTo}
        ...
        />
        <div>
          <label htmlFor="stock-report-lotNumber" className="mb-1 block text-xs font-medium text-gray-600">
            Lot n°
          </label>
          <Input
            id="stock-report-lotNumber"
            type="number"
            className="w-24"
            value={lotNumber}
        ...
        <div>
          <label htmlFor="stock-report-productId" className="mb-1 block text-xs font-medium text-gray-600">
            Produit
          </label>
          <select
            id="stock-report-productId"
        ...
        <div>
          <label htmlFor="stock-report-warehouseId" className="mb-1 block text-xs font-medium text-gray-600">
            Magasin
          </label>
          <select
            id="stock-report-warehouseId"
            className="rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500"
            value={warehouseId}
        ...
        <div>
          <label htmlFor="stock-report-direction" className="mb-1 block text-xs font-medium text-gray-600">
            Type de mouvement
          </label>
          <select
            id="stock-report-direction"
            className="rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500"
            value={direction}
      ...
      {error && <p className="text-sm text-red-600">Impossible de charger les mouvements de stock.</p>}
      {movements && (
              ...
              {movements.map((row) => {
                ...
                return (
                  <tr key={row.key} className="border-b border-gray-100">
                    ...
                    <td className="py-2 pr-3">{movementDirectionLabel(row.direction, row.type === "ADJUSTMENT")}</td>
