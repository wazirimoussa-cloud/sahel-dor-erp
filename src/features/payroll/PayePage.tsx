...
function employeeName(employees: { full_name: string } | { full_name: string }[] | null) {
            ...
            <tbody>
              {payslips.map((payslip) => (
                <tr key={payslip.id} className="border-b border-gray-100">
                  <td className="py-2">{new Date(payslip.period).toLocaleDateString("fr-FR", { month: "long", year: "numeric" })}</td>
                  <td className="py-2">{employeeName(payslip.employees)}</td>
