...
export interface JournalEntryForVat {
  journal_entry_lines: { debit: number; credit: number; chart_of_accounts: { code: string } | null }[];
}

// TVA collectée (4431) sur les ventes de la période moins TVA déductible (4452) sur les
// achats de la période -- extrait du hook (useVatDeclaration ci-dessous) pour être
// testable unitairement sans requête réseau, comme computeFinancialStatements.
export function computeVatDeclaration(entries: JournalEntryForVat[], vatRate: number) {
...
// Déclaration TVA mensuelle. Réutilise le même journal comptable que les États
// financiers (Phase 10), mais raisonne uniquement sur la période choisie (une
// déclaration ne porte jamais sur un solde cumulé depuis toujours).
export function useVatDeclaration(startDate: string, endDate: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return computeVatDeclaration(journalRes.data ?? [], companyRes.data?.vat_rate ?? 0);
    },
  });
}
