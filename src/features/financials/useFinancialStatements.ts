...
export interface ComputeFinancialStatementsInput {
  startDate: string;
  endDate: string;
...
// récupérées -- extrait du hook (useFinancialStatements ci-dessous) pour être testable
// unitairement sans requête réseau : c'est la seule logique de calcul financier de
// l'app jamais exercée par un test (les tests d'intégration existants vérifient les
// écritures comptables générées par les RPC, pas cette agrégation JS côté frontend).
export function computeFinancialStatements(input: ComputeFinancialStatementsInput) {
  const {
    startDate,
    endDate,
    products,
...
export function useFinancialStatements(startDate: string, endDate: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const fixedAssets = fixedAssetsRes.data as FixedAssetRow[];
