  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("orders")
        .select(
          "id, status, payment_status, amount_paid, created_at, clients(name), companies(vat_rate, vat_reduced_rate), order_items(quantity, unit_price, products(name, vat_exempt, vat_reduced))",
        )
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("orders")
        .select(
          "id, status, payment_status, amount_paid, created_at, company_id, user_id, warehouse_id, users(email), clients(id, name), companies(vat_rate, vat_reduced_rate), order_items(id, quantity, unit_price, products(id, name, unit, vat_exempt, vat_reduced))",
        )
