-- Corrige une limite documentée depuis le point 36 du README : les frais de transport/
-- manutention (purchases.freight_cost/handling_cost) étaient répartis au prorata de la
-- QUANTITÉ commandée -- une réception mélangeant des produits d'unités très différentes
-- (ex. tonnes de céréales à faible valeur unitaire et cartons d'un produit à forte valeur
-- unitaire) faisait porter la même quote-part par unité de quantité à chaque ligne, sans
-- pondération par valeur. Bascule sur un prorata de la VALEUR commandée (quantité × coût
-- unitaire d'achat) : une ligne représentant 80% de la valeur totale de l'achat porte 80%
-- des frais, peu importe sa part en quantité physique.
--
-- Signature de receive_purchase inchangée (create or replace suffit). Reste du
-- comportement identique à 0046_frais_a_la_creation.sql : dénominateur toujours basé sur
-- les quantités/valeurs COMMANDÉES (pas reçues -- une perte en transit ne doit pas
-- gonfler le coût des survivants), frais toujours saisis à la création par le Gérant,
-- toujours comptabilisés en Débit 608 / Crédit 521 séparément du 601.
...
declare
  ...
  v_account_521 uuid;
  v_entry_id uuid;
  v_total_ordered_value numeric(14, 2) := 0;
...
begin
  ...
  -- Valeur totale commandée (quantité × coût unitaire d'achat), dénominateur du prorata
  -- -- remplace l'ancien v_total_ordered_qty (quantité seule).
  select coalesce(sum(quantity * unit_cost), 0) into v_total_ordered_value
  ...
  loop
    ...
    -- Part de cette ligne dans la valeur totale commandée, ramenée à une quote-part par
    -- unité : (frais_total × valeur_ligne / valeur_totale) / quantité_ligne, qui se
    -- simplifie en frais_total × coût_unitaire / valeur_totale (le quantity de la ligne
    -- s'annule). Repli à 0 si la valeur totale commandée est nulle (tous les coûts
    -- unitaires à 0), symétrique à l'ancien repli sur quantité nulle.
    v_ancillary_per_unit := case
      when v_total_ordered_value > 0 then v_ancillary_total * v_item.unit_cost / v_total_ordered_value
      else 0
    end;
